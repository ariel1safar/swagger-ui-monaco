...
If NestJS already manages your Swagger UI page, keep using `@nestjs/swagger` and replace its `swagger-ui-dist` dependency. After `0.1.0-beta.1` is published, add this to your application's root `package.json`:
...
Run `npm install`, commit the updated lockfile, and restart your app. Your `SwaggerModule.setup(...)` call stays unchanged. The [wrapper integration guide](https://github.com/ariel1safar/swagger-ui-monaco/blob/main/monaco/docs/wrappers.md) covers Yarn resolutions, pnpm overrides, verification, and compatibility limits. This replaces only the distribution assets; it is not a replacement for the `swagger-ui` or `swagger-ui-react` JavaScript APIs.
...
`getAbsoluteFSPath()`, its `absolutePath` alias, the default export, and the `swagger-ui-monaco-dist/absolute-path` and `swagger-ui-monaco-dist/absolute-path.js` default exports return the installed assets directory. ESM and CommonJS entry points are provided. Node.js 22.14 or newer is required.
