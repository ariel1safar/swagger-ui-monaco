...
export async function copySwaggerAssets({ upstream, output, bootstrap }) {
  ...
  for (const file of [
    ...
    await cp(resolve(upstream, file), resolve(output, file));
  }
  const upstreamBundle = await readFile(resolve(upstream, 'swagger-ui-bundle.js'));
  const modification = Buffer.from('\n/* MODIFIED BY swagger-ui-monaco: appended bootstrap installs the Monaco integration. */\n;');
  await writeFile(resolve(output, 'swagger-ui-bundle.js'), Buffer.concat([
    upstreamBundle,
    modification,
...
async function main() {
  ...
  await copySwaggerAssets({ upstream, output: `${dist}/assets`, bootstrap: bootstrap.outputFiles[0].text });
